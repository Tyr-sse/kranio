...
const server = http.createServer((Req,Res)=>{
	...
	let u = req.url,i;
	console.log('RAW URL: ',u);
	let ext = 'none';
	for(i =u.length; i>0 ;i--){
		if(u[i]==='.'){
			ext = u.substring(i,u.length);
			u = u.substring(0,i);
			break;
		};
		if(u[i]==='/') break;
	...
	console.log('\n_______________','URL: <',u,'> ext: <',ext,'>');
	
	// ext = ext==='none'?'html':ext;
	// fs.readFile('src/match.html', 
		// (errReading, data)=>{
			// if(errReading) return;
			// console.log('<>');
	...
	// function sendHtml(fileName){
		// const fna ="src/match";
		// fs.readFile(fna, (errorReading, data)=>{
			// if(errorReading){
				// res.writeHead(501);
				// res.write('Error 501');
				// console.log('SERVER COULD NOT READ FILE ('+fileName+')');
			// }else{
				// res.writeHead(200, {'Content-type':'text/html'});
				// res.write(data);
				// //console.log('success');
